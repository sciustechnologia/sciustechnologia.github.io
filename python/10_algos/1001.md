...
1. Outer Loop (for iter_num in range(len(list)-1,0,-1):):
...
2. Inner Loop (for idx in range(iter_num):):
...
3. Comparison and Swapping (if list[idx]>list[idx+1]:):
